...
export const routeCompletion = action({
  args: {
    ...
    address: v.string(),
    keyType: v.union(v.literal("agent"), v.literal("developer")),
  },
  returns: v.object({
    content: v.string(),
    model: v.string(),
  ...
  handler: async (ctx, args) => {
    console.log("RouteCompletion called with", {
    ...
    try {
      ...
      const response = await fetch("https://api.venice.ai/v1/chat/completions", {
        ...
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${selectedProvider.veniceApiKey}`,
        },
        body: JSON.stringify({
      ...
      const pointsEarned = Math.floor(data.usage.total_tokens / 100);
      if (pointsEarned > 0) {
        await ctx.runMutation(api.providers.awardProviderPoints, {
          providerId: selectedProvider._id,
          promptsServed: 1,
          tokensProcessed: data.usage.total_tokens,
        });
      }
      ...
      throw new Error(`Failed to complete request: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  },
});
