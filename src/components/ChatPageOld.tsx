...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (showModelDropdown && !(event.target as Element).closest('.relative')) {
  ...
  // Auto-adjust intent when selected model capabilities change
  useEffect(() => {
    if (selectedModel) {
      const model = getAllAvailableModels.find(m => m.id === selectedModel);
      if (model && model.capabilities.length > 0) {
        // Check if current intent is still available for this model
        const isCurrentIntentAvailable = model.capabilities.includes(selectedIntent.type);
        if (!isCurrentIntentAvailable) {
          // Auto-select the first available intent for this model
          const availableIntents = CHAT_INTENTS.filter(intent => 
            model.capabilities.includes(intent.type)
          );
  ...
  }, [selectedModel, getAllAvailableModels, selectedIntent.type]);
  ...
  // Get available models for current intent
  const getModelsForIntent = useMemo(() => {
    if (!availableModels) return [];

    switch (selectedIntent.type) {
      case 'code':
        return availableModels.code?.length > 0 ? availableModels.code : 
               availableModels.text?.filter((m: any) => 
                 m.id.includes('coder') || m.id.includes('qwen-2.5-coder') || m.id.includes('deepseek')
               ) || [];
      case 'image':
        return availableModels.image || [];
      case 'vision':
        return availableModels.multimodal?.length > 0 ? availableModels.multimodal :
               availableModels.text?.filter((m: any) => m.id.includes('vision') || m.id.includes('vl')) || [];
      case 'audio':
        return availableModels.audio || [];
      case 'analysis':
        return availableModels.text?.filter((m: any) => 
          m.id.includes('70b') || m.id.includes('405b') || m.id.includes('qwq')
        ) || [];
      default:
        return availableModels.text || [];
    }
  }, [availableModels, selectedIntent.type]);

  const currentModelInfo = useMemo(() => {
    if (!availableModels) return null;
    const models = getModelsForIntent;
    const activeModel = selectedModel || selectedIntent.model;
    return models?.find((m: any) => m.id === activeModel) || models?.[0];
  }, [availableModels, selectedIntent, selectedModel, getModelsForIntent]);
  ...
  // Get model capabilities based on model type and characteristics
  const getModelCapabilities = (model: any): string[] => {
    ...
    if (model.type === 'image') {
      ...
      // All text models can do chat
      capabilities.push('chat');
      
      // Code capabilities
      if (model.id.includes('coder') || model.id.includes('deepseek') || model.id.includes('qwen-2.5-coder')) {
        capabilities.push('code');
      }
      
      // Analysis capabilities (large context models)
      if (model.id.includes('70b') || model.id.includes('405b') || model.id.includes('qwq') || 
  ...
  const getAllAvailableModels = useMemo(() => {
    if (!availableModels) return [];
    
    const allModels = [
      ...(availableModels.text || []).map((m: any) => ({ ...m, type: 'text' })),
      ...(availableModels.code || []).map((m: any) => ({ ...m, type: 'code' })),
      ...(availableModels.image || []).map((m: any) => ({ ...m, type: 'image' })),
      ...(availableModels.multimodal || []).map((m: any) => ({ ...m, type: 'multimodal' })),
      ...(availableModels.audio || []).map((m: any) => ({ ...m, type: 'audio' })),
    ];
  ...
  // Filter available intents based on selected model
  const getAvailableIntents = useMemo(() => {
    if (!selectedModel) {
      // If no model selected, show all intents
      return CHAT_INTENTS;
    }
    
    const model = getAllAvailableModels.find(m => m.id === selectedModel);
    if (!model) return CHAT_INTENTS;
    
    const modelCapabilities = model.capabilities;
    
    return CHAT_INTENTS.filter(intent => 
      modelCapabilities.includes(intent.type)
    );
  }, [selectedModel, getAllAvailableModels]);
      ...
      <div className="flex h-screen">
        ...
        <div className="flex-1 flex">
          <div className="flex-1 flex flex-col">
            {/* Model and Intent Selector */}
            <div className="p-4 border-b border-white/10">
              {/* Model Selector - Now Primary */}
              <div className="mb-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm text-gray-300 font-medium">Choose AI Model:</span>
                </div>
                <div className="relative">
                  <button
                    onClick={() => setShowModelDropdown(!showModelDropdown)}
                    className="flex items-center justify-between w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-lg text-sm text-white hover:bg-white/10 transition-colors"
                  >
                    <div className="flex items-center gap-2">
                      <span className="truncate max-w-[240px]">
                        {selectedModel ? formatModelName(selectedModel) : 'All Models Available'}
                      </span>
                      {selectedModel && (
                        <div className="flex gap-1">
                          {getAllAvailableModels.find(m => m.id === selectedModel)?.capabilities.map(cap => (
                            <span key={cap} className="px-1.5 py-0.5 bg-gold/20 text-gold text-xs rounded">
                  ...
                  {showModelDropdown && (
                    <div className="absolute top-full left-0 mt-1 w-full bg-black-light border border-white/20 rounded-lg shadow-xl z-50 max-h-64 overflow-y-auto">
                      <button
                        onClick={() => {
                          setSelectedModel(null);
                          setShowModelDropdown(false);
                        }}
                        className={`w-full px-3 py-3 text-left text-sm hover:bg-white/10 transition-colors border-b border-white/10 ${
                          !selectedModel ? 'bg-white/10 text-gold' : 'text-white'
                        }`}
                      >
                        <div className="font-medium">All Models Available</div>
                        <div className="text-xs text-gray-400">Choose task first, then optimal model</div>
                      </button>
                      {getAllAvailableModels.map((model: any) => (
                        <button
                          key={model.id}
                          onClick={() => {
                          ...
                          className={`w-full px-3 py-3 text-left text-sm hover:bg-white/10 transition-colors ${
                            selectedModel === model.id ? 'bg-white/10 text-gold' : 'text-white'
                          }`}
                        >
                          <div className="flex items-center justify-between">
                            <div>
                              <div className="font-medium">{formatModelName(model.id)}</div>
                              <div className="flex gap-1 mt-1">
                                {model.capabilities.map((cap: string) => (
                                  <span key={cap} className="px-1.5 py-0.5 bg-gray-600 text-gray-300 text-xs rounded">
                                    {cap === 'chat' ? '💬' : cap === 'code' ? '💻' : cap === 'image' ? '🎨' : 
                                     cap === 'analysis' ? '📊' : cap === 'vision' ? '👁️' : '🎵'}
                                  </span>
                                ))}
                            ...
                            {model.contextLength > 0 && (
                              <div className="text-xs text-gray-400">{model.contextLength.toLocaleString()}k</div>
                            )}
                          </div>
                        </button>
                      ))}
              ...
              {/* Intent Selector - Now Filtered by Model */}
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm text-gray-300 font-medium">Available Tasks:</span>
                  {selectedModel && (
                    <span className="text-xs text-gray-400">
                      ({getAvailableIntents.length} tasks available for this model)
                    </span>
                  )}
                </div>
